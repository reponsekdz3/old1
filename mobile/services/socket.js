/**
 * Advanced Socket Service with Offline Support & Reconnection
 * Handles real-time communication with automatic reconnection and message queuing
 */
import { io } from 'socket.io-client';
import { SOCKET_URL } from '../config';
import { Cache, offlineQueue } from './cache';
...
function setupSocketListeners(userId) {
  ...
  socket.on('new_message', async (data) => {
    ...
    if (data.sender_id && data.receiver_id) {
      ...
      await Cache.appendMessage(chatId, data);
    }
  ...
  socket.on('new_group_message', async (data) => {
    ...
    if (data.group_id) {
      await Cache.appendMessage(`group_${data.group_id}`, data);
    }
...
export const emitWithQueue = (event, data) => {
  return new Promise((resolve, reject) => {
    if (socket && socket.connected) {
      ...
      // Also add to offline queue for persistence
      Cache.addToOfflineQueue(event, data, 'normal');
    }
  });
};
...
export const reconnectSocket = async (userId) => {
  if (socket) {
    socket.connect();
  } else {
