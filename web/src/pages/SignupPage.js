...
const ALL_COUNTRIES = PHONE_COUNTRIES.map(c => c.name)
  ...
  return (
    <div className="mt-2 space-y-1">
      <div className="flex gap-1">
        {[0,1,2,3,4].map(i => (
          <div key={i} className={`h-1 flex-1 rounded-full transition-colors duration-300 ${i <= score-1 ? lvl.color : 'bg-gray-100'}`} />
        ))}
      </div>
...
export default function SignupPage() {
  ...
  const validateStep0 = () => {
    ...
    else if (!/^\+[1-9]\d{5,14}$/.test(form.phone_number.replace(/\s/g,'')))
      e.phone_number = 'Enter a valid international phone number';
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      toast.error(msg);
      if (msg.toLowerCase().includes('phone')) { setStep(0); setErrors({ phone_number: msg }); }
      else if (msg.toLowerCase().includes('email')) { setStep(0); setErrors({ email: msg }); }
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="flex min-h-screen overflow-hidden font-sans">
        ...
        <div className="relative z-10 mx-10 mb-8 bg-white/10 rounded-2xl p-4 border border-white/20 backdrop-blur">
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              <p className="text-white/50 text-xs mt-0.5">All your messages and calls are private.</p>
            </div>
          </div>
        </div>
      </div>
      ...
      {/* RIGHT */}
      <div className="w-full lg:w-[500px] flex flex-col bg-white relative overflow-hidden">
        <div className="lg:hidden absolute top-0 left-0 right-0 h-1.5 bg-gradient-to-r from-[#075E54] to-[#25D366]" />
        <div className="px-8 sm:px-12 py-10 max-w-md mx-auto w-full flex-1 overflow-y-auto">
                ...
                <React.Fragment key={i}>
                  <div className={`flex flex-col items-center gap-1 ${active||done ? '' : 'opacity-40'}`}>
                    <div className={`w-9 h-9 rounded-full flex items-center justify-center transition-all
                      ...
                      {done ? <FiCheck size={15} className="text-white" strokeWidth={3} />
