...
def create_status():
    try:
        ...
        # Store background color in content prefix if text status
        if background_color and content:
            status.content = f'__bg:{background_color}__' + content
...
def get_all_statuses():
    """Get statuses from all contacts + self"""
    try:
        user_id = get_jwt_identity()
        now = datetime.utcnow()
        ...
        contact_user_ids.append(user_id)  # include self
        ...
        for s in all_statuses:
            ...
            # Parse background color from content
            content = s.content
            bg_color = '#008069'
            ...
            grouped[uid]['statuses'].append({
                ...
                'viewed': User.query.get(user_id) in s.viewers,
            })
