...
const BUBBLES = [
  ...
  { id: 5, side: 'left', type: 'link', url: 'maps.google.com', title: 'Uganda National Park Guide', desc: 'Everything you need...', time: '9:45', avatar: 'A', delay: 1.2 },
        ...
        <div className={`rounded-xl px-2.5 py-1.5 shadow-sm ${isOwn ? 'bg-[#DCF8C6] rounded-tr-none' : 'bg-white rounded-tl-none'}`}>
          ...
          {bubble.type === 'image' && (
            <div className="rounded-lg overflow-hidden w-32 h-24 relative mb-0.5">
              ...
              {bubble.caption && <div className="absolute bottom-0 left-0 right-0 bg-black/40 text-white text-[9px] px-1.5 py-0.5">{bubble.caption}</div>}
            </div>
          )}
          {bubble.type === 'link' && (
            <div className="rounded-lg overflow-hidden border border-gray-100 min-w-[190px]">
              <div className="h-14 bg-gradient-to-br from-blue-400 to-teal-500 flex items-center justify-center">
                <svg className="w-6 h-6 text-white/80" fill="none" stroke="currentColor" strokeWidth={2} viewBox="0 0 24 24"><path d="M10 13a5 5 0 007.54.54l3-3a5 5 0 00-7.07-7.07l-1.72 1.71"/><path d="M14 11a5 5 0 00-7.54-.54l-3 3a5 5 0 007.07 7.07l1.71-1.71"/></svg>
              </div>
          ...
          {bubble.type === 'text' && <p className="text-sm text-gray-800 leading-snug">{bubble.text}</p>}
    ...
    <div className="flex h-screen overflow-hidden font-sans">
      ...
      <div className="hidden lg:flex flex-col flex-1 bg-[#075E54] relative overflow-hidden select-none">
        ...
        <div className="relative z-10 flex flex-col h-full max-w-sm mx-auto w-full px-0">
          <div className="bg-[#075E54] px-4 pt-8 pb-2">
      ...
      {/* ── RIGHT: Login form — fixed height, no scroll ── */}
      <div className="w-full lg:w-[460px] flex flex-col justify-center bg-white relative overflow-hidden">
        ...
        <div className="px-8 sm:px-10 py-6 max-w-md mx-auto w-full">
          {/* Logo */}
          ...
          <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }}>
            ...
            <form onSubmit={handleSubmit} className="space-y-3">
              <div>
                ...
                <div className="relative">
                  <FiPhone className="absolute left-4 top-1/2 -translate-y-1/2 text-[#25D366]" size={16} />
                  <input type="tel" value={formData.phone_number}
                    onChange={e => setFormData(p => ({ ...p, phone_number: e.target.value }))}
                    placeholder="+256 701 234 567" required autoComplete="tel"
                    className="w-full pl-11 pr-4 py-3 border-2 border-gray-200 rounded-2xl focus:outline-none focus:border-[#25D366] focus:ring-4 focus:ring-green-50 transition text-sm" />
                </div>
              </div>
              <div>
                ...
                <div className="relative">
                  ...
                  <input type={showPassword ? 'text' : 'password'} value={formData.password}
                    ...
                    className="w-full pl-11 pr-12 py-3 border-2 border-gray-200 rounded-2xl focus:outline-none focus:border-[#25D366] focus:ring-4 focus:ring-green-50 transition text-sm" />
              ...
              <motion.button whileHover={{ scale: 1.01 }} whileTap={{ scale: 0.99 }}
                ...
                className="w-full bg-[#25D366] hover:bg-[#1fbd5a] disabled:bg-gray-200 text-white font-bold py-3.5 rounded-2xl transition flex items-center justify-center gap-2 text-sm shadow-lg shadow-green-100 mt-1">
                {loading
            ...
            <button onClick={() => navigate('/signup')}
              className="w-full border-2 border-gray-200 hover:border-[#25D366] hover:bg-green-50 text-gray-700 font-semibold py-3 rounded-2xl transition text-sm flex items-center justify-center gap-2">
