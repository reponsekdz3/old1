...
import {
  FiPhone, FiLock, FiArrowRight, FiEye, FiEyeOff,
  FiCheck, FiMic, FiVideo, FiImage, FiFile, FiMapPin,
} from 'react-icons/fi';
...
const BUBBLES = [
  ...
  { id: 4, side: 'right', type: 'image', time: '9:44', delay: 0.75, status: 'delivered', caption: 'At the airport!' },
  { id: 5, side: 'left', type: 'link', url: 'maps.google.com', title: 'Uganda National Park Guide', desc: 'Everything you need for your trip...', time: '9:45', avatar: 'A', delay: 1.0 },
    ...
    >
      ...
      <div className="relative max-w-[78%]">
        <div className={`rounded-xl px-2.5 py-1.5 shadow-sm text-[11px]
          ...
          {bubble.type === 'link' && (
            <div className="rounded-lg overflow-hidden border border-gray-100 min-w-[170px]">
              ...
              <div className="p-1.5 bg-gray-50">
                ...
                <p className="text-[8px] text-gray-500 truncate">{bubble.desc}</p>
              </div>
            </div>
          )}
          ...
          {bubble.type === 'text' && (
            <p className="text-gray-800 leading-snug">{bubble.text}</p>
          )}
...
export default function LoginPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    let hasErr = false;
    if (!form.phone_number.trim()) { setPhoneError('Phone number is required'); hasErr = true; }
    if (!form.password) { setPasswordError('Password is required'); hasErr = true; }
    if (hasErr) return;
    ...
    try {
      ...
      if (msg.toLowerCase().includes('phone') || msg.toLowerCase().includes('credentials')) {
        setPhoneError('Incorrect phone number or password');
        setPasswordError(' ');
      } else if (msg.toLowerCase().includes('suspend') || msg.toLowerCase().includes('ban')) {
        toast.error(msg);
      } else {
        toast.error(msg);
      }
  ...
  return (
    <div className="flex h-screen overflow-hidden font-sans">
      {/* ── LEFT: Animated Bitese preview ── */}
      <div className="hidden lg:flex flex-col flex-1 bg-[#075E54] relative overflow-hidden select-none">
        <div className="absolute inset-0 pointer-events-none">
          <div className="absolute inset-0" style={{ backgroundImage: 'radial-gradient(circle at 20% 20%, rgba(255,255,255,0.06) 0%, transparent 50%), radial-gradient(circle at 80% 80%, rgba(255,255,255,0.04) 0%, transparent 50%)' }} />
        </div>
        ...
        <div className="relative z-10 flex flex-col h-full max-w-[340px] mx-auto w-full">
          ...
          <div className="flex-1 flex flex-col bg-[#e5ddd5] overflow-hidden min-h-0">
            ...
            <div className="flex-1 overflow-hidden px-2.5 py-2 space-y-0">
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    onChange={e => { setForm(p => ({ ...p, phone_number: e.target.value })); setPhoneError(''); }}
                    placeholder="+256 701 234 567"
                    autoComplete="tel"
                    className={`w-full pl-11 pr-4 py-3 border-2 rounded-2xl focus:outline-none focus:ring-4 transition text-sm
                      ${phoneError ? 'border-red-300 focus:border-red-400 focus:ring-red-50' : 'border-gray-200 focus:border-[#25D366] focus:ring-green-50'}`}
                  />
                </div>
                <AnimatePresence>
                  {phoneError && (
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    className={`w-full pl-11 pr-12 py-3 border-2 rounded-2xl focus:outline-none focus:ring-4 transition text-sm
                      ${passwordError && passwordError.trim() ? 'border-red-300 focus:border-red-400 focus:ring-red-50' : 'border-gray-200 focus:border-[#25D366] focus:ring-green-50'}`}
                  />
